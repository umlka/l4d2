...
static const char g_sWeaponName[5][17][] =
{
	...
	{//slot 1(副武器)
		...
		"cricket_bat",				//16 球拍
		"crowbar",					//32 撬棍
		"electric_guitar",			//64 电吉他
		"fireaxe",					//128 斧头
		"frying_pan",				//256 平底锅
		"golfclub",					//512 高尔夫球棍
		"baseball_bat",				//1024 棒球棒
		"katana",					//2048 武士刀
		"machete",					//4096 砍刀
		"tonfa",					//8192 警棍
		"riot_shield",				//16384 盾牌
		"pitchfork",				//32768 草叉
		"shovel"					//65536 铁铲
	},
	{//slot 2(投掷物)
...
public void OnPluginStart()
{
	...
	hSpawnFlowLimit.SetInt(999999999);

	g_hL4DSurvivorLimit.SetBounds(ConVarBound_Upper, true, 24.0);
	//g_hL4DSurvivorLimit.AddChangeHook(ConVarChanged_SurvivorLimit);
	//g_hSurvivorLimit.AddChangeHook(ConVarChanged_SurvivorLimit);
...
/*
public void ConVarChanged_SurvivorLimit(ConVar convar, const char[] oldValue, const char[] newValue)
{
	g_hL4DSurvivorLimit.IntValue = g_hSurvivorLimit.IntValue;
}
*/
...
{
	if(client == 0 || !IsClientInGame(client) || !IsPlayerAlive(client) || IsClientObserver(client))
		return -1;
	...
	{
		float fDifference = GetGameTime() - GetEntPropFloat(client, Prop_Send, "m_healthBufferTime");
